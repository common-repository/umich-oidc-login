 ...
 *
 */
namespace UMich_OIDC\Vendor\Jumbojett;
 ...
 */
if (!\class_exists('UMich_OIDC\\Vendor\\phpseclib3\\Crypt\\RSA') && !\class_exists('UMich_OIDC\\Vendor\\phpseclib\\Crypt\\RSA') && !\class_exists('UMich_OIDC\\Vendor\\Crypt_RSA')) {
...
class OpenIDConnectClient
{
     ...
     */
    protected $httpUpgradeInsecureRequests = \true;
    /**
    ...
    public function signOut($idToken, $redirect)
    {
        ...
        if ($redirect === null) {
            ...
            $signout_params = ['id_token_hint' => $idToken, 'post_logout_redirect_uri' => $redirect];
        }
    ...
    public function verifyLogoutTokenClaims($claims)
    {
        ...
        if (isset($claims->events)) {
            ...
            if (!isset($events['http://schemas.openid.net/event/backchannel-logout']) || !\is_object($events['http://schemas.openid.net/event/backchannel-logout'])) {
                return \false;
            }
        }
        // Validate the iss
        ...
        // Validate the aud
        if (!$claims->aud === $this->clientID || !\in_array($this->clientID, $claims->aud, \true)) {
            return \false;
        }
        // Validate the iat. At this point we can return true if it is ok
        if (isset($claims->iat) && (\is_int($claims->iat) && $claims->iat <= \time() + $this->leeway)) {
            return \true;
        }
        return \false;
    }
    ...
    public function getRedirectURL()
    {
        ...
        return \sprintf('%s://%s%s/%s', $protocol, $host, $port, \trim(\reset($explodedRequestUri), '/'));
    }
    /**
    ...
    private function requestAuthorization()
    {
        ...
        $auth_params = \array_merge($this->authParams, ['response_type' => $response_type, 'redirect_uri' => $this->getRedirectURL(), 'client_id' => $this->clientID, 'nonce' => $nonce, 'state' => $state, 'scope' => 'openid']);
        ...
        if (!empty($codeChallengeMethod) && \in_array($codeChallengeMethod, $this->getProviderConfigValue('code_challenge_methods_supported', []), \true)) {
            $codeVerifier = \bin2hex(\random_bytes(64));
            $this->setCodeVerifier($codeVerifier);
            if (!empty($this->pkceAlgs[$codeChallengeMethod])) {
                $codeChallenge = \rtrim(\strtr(\base64_encode(\hash($this->pkceAlgs[$codeChallengeMethod], $codeVerifier, \true)), '+/', '-_'), '=');
            } else {
                $codeChallenge = $codeVerifier;
            }
            $auth_params = \array_merge($auth_params, ['code_challenge' => $codeChallenge, 'code_challenge_method' => $codeChallengeMethod]);
        }
    ...
    public function requestClientCredentialsToken()
    {
        ...
        $post_data = ['grant_type' => $grant_type, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret, 'scope' => \implode(' ', $this->scopes)];
    ...
    public function requestResourceOwnerToken($bClientAuth = FALSE)
    {
        ...
        $headers = [];
        $grant_type = 'password';
        $post_data = ['grant_type' => $grant_type, 'username' => $this->authParams['username'], 'password' => $this->authParams['password'], 'scope' => \implode(' ', $this->scopes)];
    ...
    protected function requestTokens($code, $headers = array())
    {
        ...
        $token_params = ['grant_type' => $grant_type, 'code' => $code, 'redirect_uri' => $this->getRedirectURL(), 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret];
        ...
        if (!empty($ccm) && !empty($cv)) {
            ...
            $token_params = \array_merge($token_params, ['client_id' => $this->clientID, 'code_verifier' => $this->getCodeVerifier()]);
        }
        // Convert token params to string format
    ...
    public function requestTokenExchange($subjectToken, $subjectTokenType, $audience = '')
    {
        ...
        $post_data = array('grant_type' => $grant_type, 'subject_token_type' => $subjectTokenType, 'subject_token' => $subjectToken, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret, 'scope' => \implode(' ', $this->scopes));
        if (!empty($audience)) {
    ...
    public function refreshToken($refresh_token)
    {
        ...
        $token_params = ['grant_type' => $grant_type, 'refresh_token' => $refresh_token, 'client_id' => $this->clientID, 'client_secret' => $this->clientSecret, 'scope' => \implode(' ', $this->scopes)];
        # Consider Basic authentication if provider config is set this way
        if ($this->supportsAuthMethod('client_secret_basic', $token_endpoint_auth_methods_supported)) {
            $headers = ['Authorization: Basic ' . \base64_encode(\urlencode($this->clientID) . ':' . \urlencode($this->clientSecret))];
            unset($token_params['client_secret']);
    ...
    private function verifyRSAJWTsignature($hashtype, $key, $payload, $signature, $signatureType)
    {
        ...
        if (\class_exists('UMich_OIDC\\Vendor\\phpseclib3\\Crypt\\RSA', \false)) {
            $key = \UMich_OIDC\Vendor\phpseclib3\Crypt\PublicKeyLoader::load($public_key_xml)->withHash($hashtype);
            if ($signatureType === 'PSS') {
                $key = $key->withMGFHash($hashtype)->withPadding(\UMich_OIDC\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PSS);
            } else {
                $key = $key->withPadding(\UMich_OIDC\Vendor\phpseclib3\Crypt\RSA::SIGNATURE_PKCS1);
            }
            return $key->verify($payload, $signature);
        } elseif (\class_exists('UMich_OIDC\\Vendor\\Crypt_RSA', \false)) {
            $rsa = new Crypt_RSA();
            ...
            $rsa = new \UMich_OIDC\Vendor\phpseclib\Crypt\RSA();
            $rsa->setHash($hashtype);
            if ($signatureType === 'PSS') {
                $rsa->setMGFHash($hashtype);
            }
            $rsa->loadKey($public_key_xml, \UMich_OIDC\Vendor\phpseclib\Crypt\RSA::PUBLIC_FORMAT_XML);
            $rsa->setSignatureMode($signatureType === 'PSS' ? \UMich_OIDC\Vendor\phpseclib\Crypt\RSA::SIGNATURE_PSS : \UMich_OIDC\Vendor\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            return $rsa->verify($payload, $signature);
        }
    }
    /**
    ...
    private function verifyHMACJWTsignature($hashtype, $key, $payload, $signature)
    {
        ...
        $expected = \hash_hmac($hashtype, $payload, $key, \true);
    ...
    public function verifyJWTsignature($jwt)
    {
        ...
        switch ($header->alg) {
            ...
            case 'RS512':
                ...
                $verified = $this->verifyRSAJWTsignature($hashtype, $jwk, $payload, $signature, $signatureType);
                break;
            case 'HS256':
            case 'HS512':
            case 'HS384':
    ...
    protected function validateIssuer($iss)
    {
        ...
        return $iss === $this->getIssuer() || $iss === $this->getWellKnownIssuer() || $iss === $this->getWellKnownIssuer(\true);
    }
    /**
     * @param object $claims
    ...
    protected function verifyJWTclaims($claims, $accessToken = null)
    {
        if (isset($claims->at_hash) && isset($accessToken)) {
            ...
            $len = (int) $bit / 16;
            $expected_at_hash = $this->urlEncode(\substr(\hash('sha' . $bit, $accessToken, \true), 0, $len));
        }
        return $this->validateIssuer($claims->iss) && ($claims->aud === $this->clientID || \in_array($this->clientID, $claims->aud, \true)) && (!isset($claims->nonce) || $claims->nonce === $this->getNonce()) && (!isset($claims->exp) || \gettype($claims->exp) === 'integer' && $claims->exp >= \time() - $this->leeway) && (!isset($claims->nbf) || \gettype($claims->nbf) === 'integer' && $claims->nbf <= \time() + $this->leeway) && (!isset($claims->at_hash) || !isset($accessToken) || $claims->at_hash === $expected_at_hash);
    }
    /**
     * @param string $str
     * @return string
    ...
    public function introspectToken($token, $token_type_hint = '', $clientId = null, $clientSecret = null)
    {
        ...
        // Convert token params to string format
        $headers = ['Authorization: Basic ' . \base64_encode(\urlencode($clientId) . ':' . \urlencode($clientSecret)), 'Accept: application/json'];
        if ($this->supportsAuthMethod('client_secret_jwt', $token_endpoint_auth_methods_supported)) {
            $client_assertion_type = $this->getProviderConfigValue('client_assertion_type');
    ...
    {
        return \class_exists('UMich_OIDC\\Vendor\\phpseclib3\\Crypt\\RSA') || \class_exists('UMich_OIDC\\Vendor\\phpseclib\\Crypt\\RSA') || \class_exists('UMich_OIDC\\Vendor\\Crypt_RSA');
    }
    /**
    ...
    protected function getJWTClientAssertion($aud)
    {
        ...
        $payload = \json_encode(['sub' => $this->getClientID(), 'iss' => $this->getClientID(), 'aud' => $aud, 'jti' => $jti, 'exp' => $now + 3600, 'iat' => $now]);
        // Encode Header to Base64Url String
        $base64UrlHeader = $this->urlEncode($header);
        // Encode Payload to Base64Url String
        $base64UrlPayload = $this->urlEncode($payload);
        // Create Signature Hash
        $signature = \hash_hmac('sha256', $base64UrlHeader . "." . $base64UrlPayload, $this->getClientSecret(), \true);
    ...
    public function supportsAuthMethod($auth_method, $token_endpoint_auth_methods_supported)
    {
        ...
        if (!\in_array($auth_method, $this->token_endpoint_auth_methods_supported, \true)) {
            return \false;
        }
        return \in_array($auth_method, $token_endpoint_auth_methods_supported, \true);
    }
}
